...
SELECT 
   hotel_name,
   COUNT(hotel_name) as booking_on_weekends
FROM bookings
WHERE EXTRACT(DAY FROM booking_date) in (6,7)
GROUP BY hotel_name
ORDER BY booking_on_weekends DESC;


SELECT 
   hotel_name,
   SUM(IF(EXTRACT(DAY FROM booking_date) in (6,7),1,0)) as booking_on_weekends
FROM bookings
GROUP BY hotel_name
ORDER BY booking_on_weekends DESC;
...
SELECT 
   hotel_name,
   SUM(CASE WHEN EXTRACT(DAY FROM booking_date) IN (6,7) THEN 1 ELSE 0 END) as booking_on_weekends
FROM bookings
GROUP by hotel_name
ORDER BY booking_on_weekends DESC;
