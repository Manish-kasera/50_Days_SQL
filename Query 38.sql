...
(
select department,ROUND(avg(salary),2) as average_salary from employees
group by department
)

select e1.department,e1.emp_name,e1.salary,average_salary from employees e1
inner join dep_avg_salary
on e1.department = dep_avg_salary.department
and e1.salary > average_salary;
