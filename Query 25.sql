...
WITH shipable_count as
(
SELECT 
  count(address) as shipable_address_count 
FROM orders as o
INNER JOIN customers as c
ON o.cust_id = c.id
),

nonshipable_count as
(
SELECT 
    count(*) as non_shipable_address_count 
FROM orders as o
INNER JOIN customers as c
ON o.cust_id = c.id
...
SELECT 
    ROUND(shipable_address_count * 100 / (shipable_address_count + non_shipable_address_count),
            2) AS percenatge_shipable_address
FROM
    shipable_count
        JOIN
    nonshipable_count;
