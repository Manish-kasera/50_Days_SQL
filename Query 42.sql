...
CREATE TABLE uber_ride (
    ...
    -- "ride_completed", "cancelled_by_driver" or "cancelled_by_user"
);
...
SELECT count(*) FROM uber_ride
WHERE ride_status IN('cancelled_by_driver','cancelled_by_user');
...
SELECT 
    ROUND(COUNT(*) / (SELECT 
                ...
                FROM
                    uber_ride
                WHERE
                    ride_status IN ('cancelled_by_driver' , 'cancelled_by_user')) * 100,
            2) AS percentage_ride_cancelled_by_driver
FROM
    uber_ride
WHERE
