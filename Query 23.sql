...
SELECT 
	  user_id,
      purchase_date,
      RANK() OVER(PARTITION BY user_id ORDER BY purchase_date ASC) as ranka
...
(SELECT 
   ...
   a1.purchase_date ,
   a1.ranka as rank1, 
   a2.purchase_date as next_purchase_date,
   a2.ranka as rank2,
   DATEDIFF(a2.purchase_date,a1.purchase_date) as diff_date
from amazon_1_2 as a1
JOIN amazon_1_2 a2
ON a1.user_id= a2.user_id
WHERE a1.ranka=1 AND a2.ranka=2)as test
WHERE diff_date <= 7;
