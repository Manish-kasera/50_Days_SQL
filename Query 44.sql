...
WITH city_average_price as
(SELECT 
     city,
     ROUND(AVG(mkt_price),2) as average_market_price
FROM house_price
GROUP BY city
)
...
SELECT 
   *
FROM house_price 
JOIN city_average_price
WHERE city_average_price.city = house_price.city
AND  house_price.mkt_price > city_average_price.average_market_price;  
